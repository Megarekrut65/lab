...
struct info_monster
{
    ...
    info_monster(string name, unsigned int xp, int damage, double chance, string type_of_attack, my_time time_info = my_time(0, 0, 0, 0, 0, 1970), int id = -1)
    {
        this->name = name;
        this->xp = xp;
        this->damage = damage;
...
int set_id()//створює унікальний id
{
    srand(time(0));
...
void write_monsters(std::vector<int> monsters_nombers)//виводить дані монстрів на еран
{
...
void write_monster(int i)//виводить дані монстра на еран
{
    cout << "Monster found:\n\n";
...
void edit_monster()//редагує монстра
{
    int number = find_id();
    my_cls();
    if (number != -1)
...
void benchmark_mode()// режим benchmark
{
    ...
    while (true)
    {
        ...
        //
        the_start = std::chrono::high_resolution_clock::now();
        number = find_xp_damage(find_monster.xp, find_monster.damage);
        the_end = std::chrono::high_resolution_clock::now();
        duration = the_end - the_start;
