...
void find_xp_damage_menu(const vector <info_monster>&);
...
vector <int> find_xp_damage(unsigned, unsigned, const vector <info_monster>&);
...
{   
    ...
    cout << "\nEnter the maximum date and time the monster was created: ";
...
void find_xp_damage_menu(const vector<info_monster>& all_monsters)
{
    unsigned min_hp = read_hp("the minimum");
    unsigned max_damage = read_damage("the maximum");
    write_monsters_menu(find_xp_damage(min_hp, max_damage,all_monsters),all_monsters);
}
vector <int> find_xp_damage(unsigned min_hp, unsigned max_damage, const vector<info_monster>& all_monsters)//пошук монстар по рівню життя і атаки
{
    std::vector <int> numbers_monsters;
    for (unsigned i = 0; i < all_monsters.size(); i++)
    {
...
void monster_search(const vector<info_monster>& all_monsters)
{
    while (true)
    {
        ...
        case '1': find_name_menu(all_monsters);
            break;
        case '2': find_xp_damage_menu(all_monsters);
            break;
...
void demo_add_monster(unsigned delay)
{

}
void demo_show_all_monsters(unsigned delay)
{

}
void demo_find_name(unsigned delay)
{

}
void demo_find_xp_damage(unsigned delay)
{

}
void demo_find_types_time(unsigned delay)
{

}
void demo_edit_monster(unsigned delay)
{

}
void demo_delete_monster(unsigned delay)
{

}
void demo_mode()
{
    cout << "\nEnter a delay to display data (in milliseconds; normal = 1800): ";
    unsigned delay = 1800;
    while (true)
    {
        cin >> delay;
...
float measurement_hp_damage_find(vector<info_monster>& all_monsters)
{
    ...
    vector <int> number;
    the_start = std::chrono::high_resolution_clock::now();
    number = find_xp_damage(hp, damage, all_monsters);
    the_end = std::chrono::high_resolution_clock::now();
    duration = the_end - the_start;
